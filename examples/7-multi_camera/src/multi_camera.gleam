...
fn view(model: Model, ctx: tiramisu.Context(Id)) -> scene.Node(Id) {
  ...
  let pixel_size =
    float.round(9.0 +. 7.0 *. maths.sin(model.rotation *. 2.0))
  ...
  let camera_topdown =
    scene.camera(
      ...
      postprocessing: option.Some(
        ...
        |> postprocessing.add_pass(
          // Oscillating pixelate effect: 2-16 pixels
          postprocessing.pixelate(pixel_size),
        )
  ...
  let camera_firstperson =
    scene.camera(
      ...
      postprocessing: option.Some(
        ...
        |> postprocessing.add_pass(postprocessing.clear_pass(option.None))
        |> postprocessing.add_pass(postprocessing.render_pass())
        |> postprocessing.add_pass(postprocessing.vignette(
          darkness: 1.5,
          offset: 0.8,
        ))
        |> postprocessing.add_pass(postprocessing.output_pass()),
  ...
  let camera_overlay =
    scene.camera(
      ...
      look_at: option.Some(vec3.Vec3(0.0, 0.0, 0.0)),
      active: False,
      viewport: option.Some(scene.ViewPort(
        overlay_x,
        overlay_y,
        overlay_size,
        overlay_size,
      ...
      postprocessing: option.Some(
        ...
        |> postprocessing.add_pass(postprocessing.render_pass())
        |> postprocessing.add_pass(
          postprocessing.pixelate(float.round(model.rotation)),
        )
        |> postprocessing.add_pass(postprocessing.output_pass()),
      ),
    )
  ...
  let spheres =
    ...
    |> scene.empty(id: SpheresGroup, transform: transform.identity, children: _)
