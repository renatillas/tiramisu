...
-opaque glint(MOB) :: {glint, config(), command_node(MOB)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(MOC) :: {command,
        fun((named_args(), list(binary()), flags()) -> MOC),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(MOD) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> MOD),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(MOE) :: {command_node,
        gleam@option:option(internal_command(MOE)),
        gleam@dict:dict(binary(), command_node(MOE)),
        flags(),
        binary()}.

-type out(MOF) :: {out, MOF} | {help, binary()}.
...
-opaque flag(MOG) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, MOG} | {error, snag:snag()}),
        fun((flag_internals(MOG)) -> value()),
        fun((flags(), binary()) -> {ok, MOG} | {error, snag:snag()}),
        gleam@option:option(MOG)}.

-type flag_internals(MOH) :: {flag_internals,
        gleam@option:option(MOH),
        fun((binary()) -> {ok, MOH} | {error, snag:snag()})}.
...
-spec pretty_help(glint(MOO), pretty_help()) -> glint(MOO).
...
-spec with_name(glint(MOR), binary()) -> glint(MOR).
...
-spec without_exit(glint(MOU)) -> glint(MOU).
without_exit(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec as_module(glint(MOX)) -> glint(MOX).
as_module(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec with_indent_width(glint(MPA), integer()) -> glint(MPA).
...
-spec with_max_output_width(glint(MPD), integer()) -> glint(MPD).
...
-spec with_min_first_column_width(glint(MPG), integer()) -> glint(MPG).
...
-spec with_column_gap(glint(MPJ), integer()) -> glint(MPJ).
...
-spec global_help(glint(MPS), binary()) -> glint(MPS).
...
-spec map_command(command(MQH), fun((MQH) -> MQJ)) -> command(MQJ).
...
-spec command_help(binary(), fun(() -> command(MQL))) -> command(MQL).
command_help(Desc, F) ->
    _record = F(),
    {command,
        erlang:element(2, _record),
...
-spec unnamed_args(args_count(), fun(() -> command(MQO))) -> command(MQO).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(MQR))
) -> command(MQR).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(MTF)) -> value()),
    fun((flags(), binary()) -> {ok, MTF} | {error, snag:snag()}),
    fun((binary()) -> {ok, MTF} | {error, snag:snag()})
) -> flag(MTF).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, MTR} | {error, snag:snag()}),
    fun((MTR) -> {ok, MTR} | {error, snag:snag()})
) -> fun((binary()) -> {ok, MTR} | {error, snag:snag()}).
...
-spec flag_constraint(flag(MTN), fun((MTN) -> {ok, MTN} | {error, snag:snag()})) -> flag(MTN).
...
-spec flag_help(flag(MUG), binary()) -> flag(MUG).
...
-spec flag_default(flag(MUJ), MUJ) -> flag(MUJ).
...
-spec flag(
    flag(MQU),
    fun((fun((flags()) -> {ok, MQU} | {error, snag:snag()})) -> command(MQX))
) -> command(MQX).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), MUM, fun((MUM, binary(), flag_entry()) -> MUM)) -> MUM.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> MQE)) -> command(MQE).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, MUZ} | {error, snag:snag()})
) -> {ok, MUZ} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(MVC)) -> {ok, MVC} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(MVV),
    list(binary()),
    fun((command_node(MVV)) -> command_node(MVV))
) -> command_node(MVV).
...
-spec update_at(
    glint(MVP),
    list(binary()),
    fun((command_node(MVP)) -> command_node(MVP))
) -> glint(MVP).
...
-spec path_help(glint(MPO), list(binary()), binary()) -> glint(MPO).
...
-spec add(glint(MPV), list(binary()), command(MPV)) -> glint(MPV).
...
-spec group_flag(glint(MRA), list(binary()), flag(any())) -> glint(MRA).
...
?DOC(false).
-spec execute(glint(MRG), list(binary())) -> {ok, out(MRG)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(MSF), list(binary()), fun((MSF) -> any())) -> nil.
