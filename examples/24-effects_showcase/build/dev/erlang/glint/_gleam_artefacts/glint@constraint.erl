...
-spec one_of(list(MHR)) -> fun((MHR) -> {ok, MHR} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(MHU)) -> fun((MHU) -> {ok, MHU} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((MHX) -> {ok, MHX} | {error, snag:snag()})) -> fun((list(MHX)) -> {ok,
        list(MHX)} |
    {error, snag:snag()}).
each(Constraint) ->
