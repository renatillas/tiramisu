...
-define(FILEPATH, "src/glint/internal/utils.gleam").
    ...
    Lines = begin
        gleam@list:fold(
            ...
            fun(Acc, Chunk) -> case {Chunk, erlang:element(1, Acc)} of
                    {[<<"\n"/utf8>>, <<"\n"/utf8>> | Rest], [S@2 | Accs]} ->
                        {[<<S@2/binary, (erlang:list_to_binary(Rest))/binary>> |
                                Accs],
                            true};
                    ...
                    {_, [S@4 | Accs@2]} when not erlang:element(2, Acc) ->
                        {[<<S@4/binary, (erlang:list_to_binary(Chunk))/binary>> |
                                Accs@2],
                            false};

                    {_, _} ->
                        {[erlang:list_to_binary(Chunk) | erlang:element(1, Acc)],
                            false}
                end end
        )
    end,
