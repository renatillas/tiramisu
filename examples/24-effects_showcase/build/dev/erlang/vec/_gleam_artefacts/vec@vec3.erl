...
-type vec3(ANIS) :: {vec3, ANIS, ANIS, ANIS}.
...
-spec from_tuple({ANIW, ANIW, ANIW}) -> vec3(ANIW).
from_tuple(Tuple) ->
    {vec3,
...
-spec to_tuple(vec3(ANIY)) -> {ANIY, ANIY, ANIY}.
to_tuple(Vector) ->
...
-spec to_list(vec3(ANJA)) -> list(ANJA).
to_list(Vector) ->
    [erlang:element(2, Vector),
        erlang:element(3, Vector),
        erlang:element(4, Vector)].
...
-spec replace_x(vec3(ANJJ), ANJJ) -> vec3(ANJJ).
...
-spec replace_y(vec3(ANJM), ANJM) -> vec3(ANJM).
...
-spec replace_z(vec3(ANJP), ANJP) -> vec3(ANJP).
...
-spec map_x(vec3(ANJS), fun((ANJS) -> ANJS)) -> vec3(ANJS).
map_x(Vector, Fun) ->
    {vec3,
...
-spec map_y(vec3(ANJV), fun((ANJV) -> ANJV)) -> vec3(ANJV).
map_y(Vector, Fun) ->
    {vec3,
        erlang:element(2, Vector),
...
-spec map_z(vec3(ANJY), fun((ANJY) -> ANJY)) -> vec3(ANJY).
map_z(Vector, Fun) ->
    {vec3,
        erlang:element(2, Vector),
        erlang:element(3, Vector),
...
-spec swap_xy(vec3(ANKB)) -> vec3(ANKB).
swap_xy(Vector) ->
    {vec3,
        erlang:element(3, Vector),
        erlang:element(2, Vector),
...
-spec swap_xz(vec3(ANKE)) -> vec3(ANKE).
swap_xz(Vector) ->
    {vec3,
        erlang:element(4, Vector),
        erlang:element(3, Vector),
...
-spec swap_yz(vec3(ANKH)) -> vec3(ANKH).
swap_yz(Vector) ->
    {vec3,
        erlang:element(2, Vector),
        erlang:element(4, Vector),
...
-spec map(vec3(ANKK), fun((ANKK) -> ANKM)) -> vec3(ANKM).
map(Vector, Fun) ->
    {vec3,
        Fun(erlang:element(2, Vector)),
        Fun(erlang:element(3, Vector)),
...
-spec map2(vec3(ANKO), vec3(ANKQ), fun((ANKO, ANKQ) -> ANKS)) -> vec3(ANKS).
map2(A, B, Fun) ->
    {vec3,
...
-spec result(vec3({ok, ANKU} | {error, ANKV})) -> {ok, vec3(ANKU)} |
    {error, ANKV}.
result(Vector) ->
    case Vector of
        {vec3, {ok, X}, {ok, Y}, {ok, Z}} ->
