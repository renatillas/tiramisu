...
-define(FILEPATH, "src/gleam_community/maths.gleam").
...
find_divisors(N) ->
    ...
    Sqrt_result@1 = case gleam@float:square_root(Nabs) of
    ...
    Max = erlang:round(Sqrt_result@1) + 1,
...
cartesian_to_polar(X, Y) ->
    R@1 = case gleam@float:square_root((X * X) + (Y * Y)) of
        {ok, R} -> R;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
logarithm(X, Base) ->
    case ((X > +0.0) andalso (Base > +0.0)) andalso (Base /= 1.0) of
        true ->
            Numerator@1 = case logarithm_10(X) of
            ...
            Denominator@1 = case logarithm_10(Base) of
            ...
            {ok, case Denominator@1 of
                    +0.0 -> +0.0;
                    -0.0 -> -0.0;
                    Gleam@denominator -> Numerator@1 / Gleam@denominator
                end};
...
golden_ratio() ->
    Sqrt5@1 = case gleam@float:square_root(5.0) of
        {ok, Sqrt5} -> Sqrt5;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
round_to_zero(X, P) ->
    P@2 = case gleam@float:power(10.0, erlang:float(P)) of
        {ok, P@1} -> P@1;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
    ...
    case P@2 of
        +0.0 -> +0.0;
        -0.0 -> -0.0;
        Gleam@denominator -> truncate_float(X * P@2) / Gleam@denominator
    end.
...
round_down(X, P) ->
    ...
    case P@2 of
        +0.0 -> +0.0;
        -0.0 -> -0.0;
        Gleam@denominator -> math:floor(X * P@2) / Gleam@denominator
    end.
...
round_up(X, P) ->
    ...
    case P@2 of
        +0.0 -> +0.0;
        -0.0 -> -0.0;
        Gleam@denominator -> math:ceil(X * P@2) / Gleam@denominator
    end.
...
round_to_nearest(X, P) ->
    ...
    Xabs = gleam@float:absolute_value(X) * P@2,
...
-spec minmax(GEU, GEU, fun((GEU, GEU) -> gleam@order:order())) -> {GEU, GEU}.
minmax(X, Y, Compare) ->
    case Compare(X, Y) of
        lt ->
            {X, Y};
...
-spec list_minimum(list(GEW), fun((GEW, GEW) -> gleam@order:order())) -> {ok,
        GEW} |
    {error, nil}.
...
-spec list_maximum(list(GFA), fun((GFA, GFA) -> gleam@order:order())) -> {ok,
        GFA} |
    {error, nil}.
...
-spec arg_minimum(list(GFE), fun((GFE, GFE) -> gleam@order:order())) -> {ok,
        list(integer())} |
    {error, nil}.
arg_minimum(Arr, Compare) ->
    case Arr of
        ...
        _ ->
            Min@1 = case list_minimum(Arr, Compare) of
                {ok, Min} -> Min;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
            ...
            {ok,
                begin
                    _pipe = gleam@list:index_map(
                        Arr,
                        fun(Element, Index) -> case Compare(Element, Min@1) of
                                eq ->
                                    Index;
...
-spec arg_maximum(list(GFJ), fun((GFJ, GFJ) -> gleam@order:order())) -> {ok,
        list(integer())} |
    {error, nil}.
arg_maximum(Arr, Compare) ->
    case Arr of
        ...
        _ ->
            Max@1 = case list_maximum(Arr, Compare) of
                {ok, Max} -> Max;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
            ...
            {ok,
                begin
                    _pipe = gleam@list:index_map(
                        Arr,
                        fun(Element, Index) -> case Compare(Element, Max@1) of
                                eq ->
                                    Index;
...
-spec extrema(list(GFO), fun((GFO, GFO) -> gleam@order:order())) -> {ok,
        {GFO, GFO}} |
    {error, nil}.
extrema(Arr, Compare) ->
    case Arr of
...
permutation_with_repetitions(N, K) ->
    case {N, K} of
        ...
        {_, _} ->
            ...
            Result@1 = case gleam@float:power(N_float, K_float) of
                {ok, Result} -> Result;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
-spec list_combination(list(GGC), integer()) -> {ok,
        gleam@yielder:yielder(list(GGC))} |
    {error, nil}.
...
-spec do_list_combination_with_repetitions(
    gleam@yielder:yielder(GGT),
    integer(),
    list(GGT)
) -> gleam@yielder:yielder(list(GGT)).
...
-spec list_combination_with_repetitions(list(GGN), integer()) -> {ok,
        gleam@yielder:yielder(list(GGN))} |
    {error, nil}.
...
-spec remove_first_by_index(gleam@yielder:yielder({integer(), GGY}), integer()) -> gleam@yielder:yielder({integer(),
    GGY}).
...
-spec do_list_permutation_without_repetitions(
    gleam@yielder:yielder({integer(), GHH}),
    integer()
) -> gleam@yielder:yielder(list(GHH)).
...
-spec list_permutation(list(GHB), integer()) -> {ok,
        gleam@yielder:yielder(list(GHB))} |
    {error, nil}.
...
-spec do_list_permutation_with_repetitions(
    gleam@yielder:yielder({integer(), GHR}),
    integer()
...
-spec list_permutation_with_repetitions(list(GHL), integer()) -> {ok,
...
-spec cartesian_product(gleam@set:set(GHV), gleam@set:set(GHX)) -> gleam@set:set({GHV,
    GHX}).
            ...
            case P of
                ...
                _ when P < +0.0 ->
                    Aggregate = gleam@list:try_fold(
                        ...
                        fun(Acc@1, Element@1) -> case Element@1 of
                                ...
                                _ ->
                                    Result@1 = case gleam@float:power(
                    ...
                    case Aggregate of
                        {ok, Result@2} ->
                            gleam@float:power(Result@2, case P of
                                    +0.0 -> +0.0;
                                    -0.0 -> -0.0;
                                    Gleam@denominator -> 1.0 / Gleam@denominator
                                end);
                ...
                _ ->
                    Aggregate@1 = gleam@list:fold(
                        ...
                        +0.0,
                        fun(Acc@2, Element@2) ->
                            Result@4 = case gleam@float:power(
                                gleam@float:absolute_value(Element@2),
                                P
                            ) of
                                {ok, Result@3} -> Result@3;
                                _assert_fail@1 ->
                                    erlang:error(#{gleam_error => let_assert,
                                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                                                file => <<?FILEPATH/utf8>>,
                            ...
                            Aggregate = gleam@list:try_fold(
                                ...
                                fun(Acc@1, Tuple@2) -> case Tuple@2 of
                                        ...
                                        _ ->
                                            ...
                                            {ok,
                                                (erlang:element(2, Tuple@2) * Result@1)
                                                + Acc@1}
                                    end end
                            ),
                            case Aggregate of
                        ...
                        _ ->
                            Aggregate@1 = gleam@list:fold(
                                ...
                                fun(Acc@2, Tuple@3) ->
                                    ...
                                    (erlang:element(2, Tuple@3) * Result@4) + Acc@2
                                end
                            ),
                            gleam@float:power(Aggregate@1, case P of
                                    +0.0 -> +0.0;
    ...
    case {Arr, N} of
        ...
        {_, N@1} when N@1 > 1 ->
            ...
            Result = gleam@list:try_fold(
                ...
                +0.0,
                fun(Acc, A) ->
                    case gleam@float:power(A - M1@1, erlang:float(N@1)) of
                        {error, _} ->
                            {error, nil};
...
standard_deviation(Arr, Ddof) ->
    case {Arr, Ddof} of
        ...
        {_, _} ->
            Variance@1 = case variance(Arr, Ddof) of
            ...
            gleam@float:square_root(Variance@1)
    end.
        ...
        true ->
            ...
            A = begin
                _pipe = gleam@list:map(
                    Arr,
                    fun(Tuple) ->
                        (erlang:element(1, Tuple) - Xmean@1) * (erlang:element(
                            2,
                            Tuple
                        )
                        - Ymean@1)
            ...
            Value@1 = case gleam@float:square_root(B * C) of
                {ok, Value} -> Value;
                _assert_fail@2 ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
-spec tversky_index(gleam@set:set(GLB), gleam@set:set(GLB), float(), float()) -> {ok,
        float()} |
    {error, nil}.
...
-spec jaccard_index(gleam@set:set(GKV), gleam@set:set(GKV)) -> float().
jaccard_index(Xset, Yset) ->
    Result@1 = case tversky_index(Xset, Yset, 1.0, 1.0) of
        {ok, Result} -> Result;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
-spec sorensen_dice_coefficient(gleam@set:set(GKY), gleam@set:set(GKY)) -> float().
sorensen_dice_coefficient(Xset, Yset) ->
    Result@1 = case tversky_index(Xset, Yset, 0.5, 0.5) of
        {ok, Result} -> Result;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
-spec overlap_coefficient(gleam@set:set(GLG), gleam@set:set(GLG)) -> float().
...
cosine_similarity(Arr) ->
    case Arr of
        ...
        _ ->
            ...
            Xarr_norm@1 = case norm(Xarr, 2.0) of
                {ok, Xarr_norm} -> Xarr_norm;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                                file => <<?FILEPATH/utf8>>,
            ...
            Yarr_norm@1 = case norm(Yarr, 2.0) of
                {ok, Yarr_norm} -> Yarr_norm;
                _assert_fail@1 ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                                file => <<?FILEPATH/utf8>>,
            ...
            Denominator = (Xarr_norm@1 * Yarr_norm@1),
        ...
        _ ->
            ...
            case Weight_is_negative of
                false ->
                    ...
                    Xarr_norm@1 = case norm_with_weights(Xarr, 2.0) of
                        {ok, Xarr_norm} -> Xarr_norm;
                        _assert_fail ->
                            erlang:error(#{gleam_error => let_assert,
                                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                    ...
                    Yarr_norm@1 = case norm_with_weights(Yarr, 2.0) of
                        {ok, Yarr_norm} -> Yarr_norm;
                        _assert_fail@1 ->
                            erlang:error(#{gleam_error => let_assert,
                                        message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
incomplete_gamma(A, X) ->
    case (A > +0.0) andalso (X >= +0.0) of
        true ->
            V@1 = case gleam@float:power(X, A) of
                {ok, V} -> V;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
            ...
            {ok,
                (V@1 * exponential(-1.0 * X)) * incomplete_gamma_sum(
                    A,
                    X,
                    case A of
                        +0.0 -> +0.0;
...
logarithmic_space(Start, Stop, Steps, Endpoint, Base) ->
    case (Steps > 0) andalso (Base > +0.0) of
        true ->
            Linspace@1 = case linear_space(Start, Stop, Steps, Endpoint) of
            ...
            {ok,
                begin
                    gleam@list:map(
                        Linspace@1,
                        fun(Value) ->
                            Result@1 = case gleam@float:power(Base, Value) of
                                {ok, Result} -> Result;
                                _assert_fail@1 ->
                                    erlang:error(#{gleam_error => let_assert,
                                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
yield_logarithmic_space(Start, Stop, Steps, Endpoint, Base) ->
    case (Steps > 0) andalso (Base > +0.0) of
        ...
        true ->
            Linspace@1 = case yield_linear_space(Start, Stop, Steps, Endpoint) of
                {ok, Linspace} -> Linspace;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
...
geometric_space(Start, Stop, Steps, Endpoint) ->
    case ((Start =< +0.0) orelse (Stop =< +0.0)) orelse (Steps < 0) of
        ...
        false ->
            Log_start@1 = case logarithm_10(Start) of
                {ok, Log_start} -> Log_start;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                                file => <<?FILEPATH/utf8>>,
            ...
            Log_stop@1 = case logarithm_10(Stop) of
                {ok, Log_stop} -> Log_stop;
                _assert_fail@1 ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
                                file => <<?FILEPATH/utf8>>,
            ...
            logarithmic_space(Log_start@1, Log_stop@1, Steps, Endpoint, 10.0)
    end.
...
gamma_lanczos(X) ->
    case X < 0.5 of
        ...
        false ->
            ...
            T = (Z + 7.0) + 0.5,
            V1@1 = case gleam@float:power(2.0 * pi(), 0.5) of
                {ok, V1} -> V1;
                _assert_fail ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
            ...
            V2@1 = case gleam@float:power(T, Z + 0.5) of
                {ok, V2} -> V2;
                _assert_fail@1 ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Pattern match failed, no pattern matched the value."/utf8>>,
            ...
            ((V1@1 * V2@1) * exponential(-1.0 * T)) * X@1
    end.
