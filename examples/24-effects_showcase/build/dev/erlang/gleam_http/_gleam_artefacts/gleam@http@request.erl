...
-spec set_header(request(JBA), binary(), binary()) -> request(JBA).
...
-spec prepend_header(request(JBD), binary(), binary()) -> request(JBD).
...
-spec set_body(request(any()), JBI) -> request(JBI).
set_body(Req, Body) ->
    {request,
...
-spec map(request(JBK), fun((JBK) -> JBM)) -> request(JBM).
...
-spec set_query(request(JBW), list({binary(), binary()})) -> request(JBW).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(JCA), gleam@http:method()) -> request(JCA).
...
-spec set_scheme(request(JCH), gleam@http:scheme()) -> request(JCH).
...
-spec set_host(request(JCK), binary()) -> request(JCK).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(JCN), integer()) -> request(JCN).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(JCQ), binary()) -> request(JCQ).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(JCT), binary(), binary()) -> request(JCT).
...
-spec remove_cookie(request(JCZ), binary()) -> request(JCZ).
