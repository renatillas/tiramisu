...
-type response(JKU) :: {response, integer(), list({binary(), binary()}), JKU}.
...
-spec set_header(response(JLJ), binary(), binary()) -> response(JLJ).
...
-spec prepend_header(response(JLM), binary(), binary()) -> response(JLM).
...
-spec set_body(response(any()), JLR) -> response(JLR).
...
-spec try_map(response(JKV), fun((JKV) -> {ok, JKX} | {error, JKY})) -> {ok,
        response(JKX)} |
    {error, JKY}.
...
-spec map(response(JLT), fun((JLT) -> JLV)) -> response(JLV).
...
-spec expire_cookie(response(JME), binary(), gleam@http@cookie:attributes()) -> response(JME).
