...
-spec map_response_body(
    fun((JOQ) -> gleam@http@response:response(JOR)),
    fun((JOR) -> JOT)
) -> fun((JOQ) -> gleam@http@response:response(JOT)).
...
-spec prepend_response_header(
    fun((JOV) -> gleam@http@response:response(JOW)),
    binary(),
    binary()
) -> fun((JOV) -> gleam@http@response:response(JOW)).
...
-spec ensure_post(gleam@http@request:request(JOZ)) -> {ok,
        gleam@http@request:request(JOZ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(JPI)) -> JPK)) -> fun((gleam@http@request:request(JPI)) -> JPK).
