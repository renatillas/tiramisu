...
-opaque next(ACOT, ACOU) :: {continue,
        ACOT,
        gleam@option:option(gleam@erlang@process:selector(ACOU))} |
    normal_stop |
...
-opaque builder(ACOV, ACOW) :: {builder,
        integer(),
        fun((gleam@http@request:request(ACOV)) -> gleam@http@response:response(ACOW)),
...
-type websocket_message(ACOX) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, ACOX}.
...
-spec continue(ACOY) -> next(ACOY, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(ACPC, ACPD), gleam@erlang@process:selector(ACPD)) -> next(ACPC, ACPD).
...
-spec convert_next(next(ACPR, ACPS)) -> mist@internal@next:next(ACPR, ACPS).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(ACQT)) -> gleam@http@response:response(ACQV))
) -> builder(ACQT, ACQV).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(ACQZ, ACRA), integer()) -> builder(ACQZ, ACRA).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), ACRF),
    integer(),
    gleam@http@response:response(ACRF)
) -> builder(mist@internal@http:connection(), ACRF).
...
-spec bind(builder(ACRR, ACRS), binary()) -> builder(ACRR, ACRS).
...
-spec with_ipv6(builder(ACRX, ACRY)) -> builder(ACRX, ACRY).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ACSD, ACSE), binary(), binary()) -> builder(ACSD, ACSE).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(ACST)
) -> {ok, websocket_message(ACST)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((ACSZ, websocket_message(ACTA), mist@internal@websocket:websocket_connection()) -> next(ACSZ, ACTA)),
    fun((mist@internal@websocket:websocket_connection()) -> {ACSZ,
        gleam@option:option(gleam@erlang@process:selector(ACTA))}),
    fun((ACSZ) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(ACTO)) -> {ok,
            gleam@otp@actor:initialised(ACTQ, ACTO, any())} |
        {error, binary()}),
    fun((ACTQ, ACTO, s_s_e_connection()) -> gleam@otp@actor:next(ACTQ, ACTO))
) -> gleam@http@response:response(response_data()).
