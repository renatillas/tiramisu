...
-type valid_message(ACAJ) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, ACAJ}.

-type websocket_message(ACAK) :: {valid, valid_message(ACAK)} | invalid.
...
-type handler_message(ACAL) :: {internal, gramps@websocket:frame()} |
    {user, ACAL}.

-type websocket_state(ACAM) :: {websocket_state,
        bitstring(),
        ACAM,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(ACBS))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(ACBS))).
...
-spec apply_frames(
    ...
    fun((ACBI, handler_message(ACBJ), websocket_connection()) -> mist@internal@next:next(ACBI, ACBJ)),
    websocket_connection(),
    mist@internal@next:next(ACBI, websocket_message(ACBJ)),
    fun((ACBI) -> nil)
) -> mist@internal@next:next(ACBI, websocket_message(ACBJ)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {ACAV,
        gleam@option:option(gleam@erlang@process:selector(ACAW))}),
    fun((ACAV) -> nil),
    fun((ACAV, handler_message(ACAW), websocket_connection()) -> mist@internal@next:next(ACAV, ACAW)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(ACAW)))} |
    {error, nil}.
