...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EIU)) -> {ok, EIU} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EIO)) -> {ok, EIO} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EIK)) -> {ok, EIK} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EJA), fun((EJA) -> json())) -> json().
...
-spec array(list(EJE), fun((EJE) -> json())) -> json().
