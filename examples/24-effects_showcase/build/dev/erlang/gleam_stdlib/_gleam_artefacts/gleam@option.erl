...
-type option(HJ) :: {some, HJ} | none.

-file("src/gleam/option.gleam", 59).
-spec reverse_and_prepend(list(HY), list(HY)) -> list(HY).
...
-spec all_loop(list(option(HP)), list(HP)) -> option(list(HP)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(HK))) -> option(list(HK)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(IG), IJ) -> {ok, IG} | {error, IJ}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, IM} | {error, any()}) -> option(IM).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(IT), fun(() -> IT)) -> IT.
...
-spec map(option(IV), fun((IV) -> IX)) -> option(IX).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(IZ))) -> option(IZ).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(JD), fun((JD) -> option(JF))) -> option(JF).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(JI), option(JI)) -> option(JI).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(JM), fun(() -> option(JM))) -> option(JM).
...
-spec values_loop(list(option(JU)), list(JU)) -> list(JU).
...
-spec values(list(option(JQ))) -> list(JQ).
values(Options) ->
    values_loop(Options, []).
