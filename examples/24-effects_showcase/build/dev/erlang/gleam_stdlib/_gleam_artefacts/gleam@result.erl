...
-spec map({ok, CQV} | {error, CQW}, fun((CQV) -> CQZ)) -> {ok, CQZ} |
    {error, CQW}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CRC} | {error, CRD}, fun((CRD) -> CRG)) -> {ok, CRC} |
    {error, CRG}.
...
-spec flatten({ok, {ok, CRJ} | {error, CRK}} | {error, CRK}) -> {ok, CRJ} |
    {error, CRK}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CRR} | {error, CRS}, fun((CRR) -> {ok, CRV} | {error, CRS})) -> {ok,
        CRV} |
    {error, CRS}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CSA} | {error, CSB}, fun((CSA) -> {ok, CSE} | {error, CSB})) -> {ok,
        CSE} |
    {error, CSB}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CSJ} | {error, any()}, CSJ) -> CSJ.
...
-spec lazy_unwrap({ok, CSN} | {error, any()}, fun(() -> CSN)) -> CSN.
...
-spec unwrap_error({ok, any()} | {error, CSS}, CSS) -> CSS.
...
-spec unwrap_both({ok, CSV} | {error, CSV}) -> CSV.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CSY} | {error, CSZ}, {ok, CSY} | {error, CSZ}) -> {ok, CSY} |
    {error, CSZ}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CTG} | {error, CTH}, fun(() -> {ok, CTG} | {error, CTH})) -> {ok,
        CTG} |
    {error, CTH}.
...
-spec all(list({ok, CTO} | {error, CTP})) -> {ok, list(CTO)} | {error, CTP}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 353).
-spec partition_loop(list({ok, CUD} | {error, CUE}), list(CUD), list(CUE)) -> {list(CUD),
    list(CUE)}.
...
-spec partition(list({ok, CTW} | {error, CTX})) -> {list(CTW), list(CTX)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CUM}, CUP) -> {ok, CUP} | {error, CUM}.
...
-spec replace_error({ok, CUS} | {error, any()}, CUW) -> {ok, CUS} | {error, CUW}.
...
-spec values(list({ok, CUZ} | {error, any()})) -> list(CUZ).
values(Results) ->
