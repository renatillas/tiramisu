...
-opaque decoder(BZD) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BZD, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BZL)) -> {ok, BZL} |
...
-spec map(decoder(CDJ), fun((CDJ) -> CDL)) -> decoder(CDL).
...
-spec then(decoder(CDV), fun((CDV) -> decoder(CDX))) -> decoder(CDX).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CEA), list(decoder(CEA))) -> decoder(CEA).
...
-spec recursive(fun(() -> decoder(CEQ))) -> decoder(CEQ).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CDF)) -> decoder(gleam@option:option(CDF)).
optional(Inner) ->
...
-spec collapse_errors(decoder(CDS), binary()) -> decoder(CDS).
...
-spec failure(CEK, binary()) -> decoder(CEK).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CEM} | {error, CEM})
) -> decoder(CEM).
...
-spec fold_dict(
    {gleam@dict:dict(CCR, CCS), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CCR, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CCS, list(decode_error())})
) -> {gleam@dict:dict(CCR, CCS), list(decode_error())}.
...
-spec dict(decoder(CCK), decoder(CCM)) -> decoder(gleam@dict:dict(CCK, CCM)).
dict(Key, Value) ->
...
-spec list(decoder(CBY)) -> decoder(list(CBY)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({CAH, list(decode_error())}, list(any())) -> {CAH,
    list(decode_error())}.
...
-spec index(
    list(BZV),
    list(BZV),
    fun((gleam@dynamic:dynamic_()) -> {BZY, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BZV)) -> {BZY, list(decode_error())})
...
-spec subfield(list(any()), decoder(BZG), fun((BZG) -> decoder(BZI))) -> decoder(BZI).
...
-spec at(list(any()), decoder(BZS)) -> decoder(BZS).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(CAQ), fun((CAQ) -> decoder(CAS))) -> decoder(CAS).
...
-spec optional_field(any(), CAW, decoder(CAW), fun((CAW) -> decoder(CAY))) -> decoder(CAY).
...
-spec optionally_at(list(any()), CBD, decoder(CBD)) -> decoder(CBD).
