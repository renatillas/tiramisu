...
-opaque set(DAE) :: {set, gleam@dict:dict(DAE, list(nil))}.
...
-spec contains(set(DAP), DAP) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(DAR), DAR) -> set(DAR).
delete(Set, Member) ->
...
-spec fold(set(DBA), DBC, fun((DBC, DBA) -> DBC)) -> DBC.
...
-spec filter(set(DBD), fun((DBD) -> boolean())) -> set(DBD).
...
-spec drop(set(DBK), list(DBK)) -> set(DBK).
drop(Set, Disallowed) ->
...
-spec take(set(DBO), list(DBO)) -> set(DBO).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DBW), set(DBW)) -> {set(DBW), set(DBW)}.
order(First, Second) ->
...
-spec intersection(set(DCB), set(DCB)) -> set(DCB).
...
-spec difference(set(DCF), set(DCF)) -> set(DCF).
...
-spec is_subset(set(DCJ), set(DCJ)) -> boolean().
...
-spec is_disjoint(set(DCM), set(DCM)) -> boolean().
...
-spec each(set(DCT), fun((DCT) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(DAM), DAM) -> set(DAM).
insert(Set, Member) ->
...
-spec from_list(list(DAX)) -> set(DAX).
from_list(Members) ->
...
-spec map(set(DBG), fun((DBG) -> DBI)) -> set(DBI).
map(Set, Fun) ->
...
-spec union(set(DBS), set(DBS)) -> set(DBS).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DCP), set(DCP)) -> set(DCP).
