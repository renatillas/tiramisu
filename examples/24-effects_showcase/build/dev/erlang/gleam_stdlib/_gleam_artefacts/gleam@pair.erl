...
-spec map_first({CQE, CQF}, fun((CQE) -> CQG)) -> {CQG, CQF}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CQH, CQI}, fun((CQI) -> CQJ)) -> {CQH, CQJ}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
