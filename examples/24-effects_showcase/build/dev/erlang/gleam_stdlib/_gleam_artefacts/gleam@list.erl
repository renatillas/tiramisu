...
-spec count_loop(list(AAW), fun((AAW) -> boolean()), integer()) -> integer().
...
-spec group_loop(list(ABY), fun((ABY) -> ACA), gleam@dict:dict(ACA, list(ABY))) -> gleam@dict:dict(ACA, list(ABY)).
...
-spec group(list(ABS), fun((ABS) -> ABU)) -> gleam@dict:dict(ABU, list(ABS)).
group(List, Key) ->
    group_loop(List, Key, maps:new()).

-file("src/gleam/list.gleam", 339).
-spec filter_loop(list(ACK), fun((ACK) -> boolean()), list(ACK)) -> list(ACK).
...
-spec filter_map(list(ACO), fun((ACO) -> {ok, ACQ} | {error, any()})) -> list(ACQ).
filter_map(List, Fun) ->
    filter_map_loop(List, Fun, []).

-file("src/gleam/list.gleam", 402).
-spec map_loop(list(ADH), fun((ADH) -> ADJ), list(ADJ)) -> list(ADJ).
...
-spec map2_loop(list(ADS), list(ADU), fun((ADS, ADU) -> ADW), list(ADW)) -> list(ADW).
...
-spec map2(list(ADM), list(ADO), fun((ADM, ADO) -> ADQ)) -> list(ADQ).
map2(List1, List2, Fun) ->
    map2_loop(List1, List2, Fun, []).

-file("src/gleam/list.gleam", 462).
-spec map_fold_loop(list(AEE), fun((AEG, AEE) -> {AEG, AEH}), AEG, list(AEH)) -> {AEG,
    list(AEH)}.
...
-spec map_fold(list(ADZ), AEB, fun((AEB, ADZ) -> {AEB, AEC})) -> {AEB,
    list(AEC)}.
...
-spec index_map(list(AEK), fun((AEK, integer()) -> AEM)) -> list(AEM).
index_map(List, Fun) ->
    index_map_loop(List, Fun, 0, []).

-file("src/gleam/list.gleam", 548).
-spec try_map_loop(list(AFC), fun((AFC) -> {ok, AFE} | {error, AFF}), list(AFE)) -> {ok,
        list(AFE)} |
    {error, AFF}.
...
-spec take_loop(list(AFS), integer(), list(AFS)) -> list(AFS).
...
-spec take(list(AFP), integer()) -> list(AFP).
take(List, N) ->
    take_loop(List, N, []).
...
-spec append_loop(list(AGE), list(AGE)) -> list(AGE).
...
-spec flatten_loop(list(list(AGP)), list(AGP)) -> list(AGP).
...
-spec flatten(list(list(AGL))) -> list(AGL).
flatten(Lists) ->
    lists:append(Lists).
...
-spec zip_loop(list(AIS), list(AIU), list({AIS, AIU})) -> list({AIS, AIU}).
...
-spec zip(list(AIN), list(AIP)) -> list({AIN, AIP}).
zip(List, Other) ->
    zip_loop(List, Other, []).

-file("src/gleam/list.gleam", 1107).
-spec strict_zip_loop(list(AJF), list(AJH), list({AJF, AJH})) -> {ok,
        list({AJF, AJH})} |
    {error, nil}.
...
-spec unzip_loop(list({AJS, AJT}), list(AJS), list(AJT)) -> {list(AJS),
    list(AJT)}.
...
-spec unzip(list({AJN, AJO})) -> {list(AJN), list(AJO)}.
unzip(Input) ->
    unzip_loop(Input, [], []).

-file("src/gleam/list.gleam", 1173).
-spec intersperse_loop(list(AKC), AKC, list(AKC)) -> list(AKC).
...
-spec intersperse(list(AJZ), AJZ) -> list(AJZ).
...
-spec unique_loop(list(AKJ), gleam@dict:dict(AKJ, nil), list(AKJ)) -> list(AKJ).
...
-spec sort(list(AKP), fun((AKP, AKP) -> gleam@order:order())) -> list(AKP).
sort(List, Compare) ->
    case List of
        [] ->
            [];
...
-spec repeat_loop(AME, integer(), list(AME)) -> list(AME).
...
-spec repeat(AMC, integer()) -> list(AMC).
repeat(A, Times) ->
    repeat_loop(A, Times, []).

-file("src/gleam/list.gleam", 1556).
-spec split_loop(list(AML), integer(), list(AML)) -> {list(AML), list(AML)}.
...
-spec split_while_loop(list(AMU), fun((AMU) -> boolean()), list(AMU)) -> {list(AMU),
    list(AMU)}.
...
-spec key_find(list({AMZ, ANA}), AMZ) -> {ok, ANA} | {error, nil}.
...
-spec key_filter(list({ANE, ANF}), ANE) -> list(ANF).
...
-spec key_pop_loop(list({ANO, ANP}), ANO, list({ANO, ANP})) -> {ok,
        {ANP, list({ANO, ANP})}} |
    {error, nil}.
...
-spec key_pop(list({ANI, ANJ}), ANI) -> {ok, {ANJ, list({ANI, ANJ})}} |
    {error, nil}.
key_pop(List, Key) ->
    key_pop_loop(List, Key, []).

-file("src/gleam/list.gleam", 1737).
-spec key_set_loop(list({ANZ, AOA}), ANZ, AOA, list({ANZ, AOA})) -> list({ANZ,
    AOA}).
...
-spec key_set(list({ANV, ANW}), ANV, ANW) -> list({ANV, ANW}).
...
-spec partition_loop(list(BIA), fun((BIA) -> boolean()), list(BIA), list(BIA)) -> {list(BIA),
    list(BIA)}.
...
-spec partition(list(AOP), fun((AOP) -> boolean())) -> {list(AOP), list(AOP)}.
partition(List, Categorise) ->
    partition_loop(List, Categorise, [], []).

-file("src/gleam/list.gleam", 1904).
-spec window_loop(list(list(APW)), list(APW), integer()) -> list(list(APW)).
...
-spec window(list(APS), integer()) -> list(list(APS)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(AQC)) -> list({AQC, AQC}).
window_by_2(List) ->
...
-spec drop_while(list(AQF), fun((AQF) -> boolean())) -> list(AQF).
...
-spec take_while_loop(list(AQL), fun((AQL) -> boolean()), list(AQL)) -> list(AQL).
...
-spec take_while(list(AQI), fun((AQI) -> boolean())) -> list(AQI).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src/gleam/list.gleam", 2002).
-spec chunk_loop(list(AQU), fun((AQU) -> AQW), AQW, list(AQU), list(list(AQU))) -> list(list(AQU)).
...
-spec chunk(list(AQP), fun((AQP) -> any())) -> list(list(AQP)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec sized_chunk(list(ARC), integer()) -> list(list(ARC)).
...
-spec reduce(list(ARN), fun((ARN, ARN) -> ARN)) -> {ok, ARN} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ARV), ARX, list(ARX), fun((ARX, ARV) -> ARX)) -> list(ARX).
...
-spec scan(list(ARR), ART, fun((ART, ARR) -> ART)) -> list(ART).
...
-spec last(list(ASA)) -> {ok, ASA} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ASE), integer()) -> list(list(ASE)).
...
-spec combination_pairs_loop(list(ASL), list({ASL, ASL})) -> list({ASL, ASL}).
...
-spec combination_pairs(list(ASI)) -> list({ASI, ASI}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src/gleam/list.gleam", 2252).
-spec take_firsts(list(list(ATF)), list(ATF), list(list(ATF))) -> {list(ATF),
    list(list(ATF))}.
...
-spec transpose_loop(list(list(ASY)), list(list(ASY))) -> list(list(ASY)).
...
-spec transpose(list(list(AST))) -> list(list(AST)).
...
-spec interleave(list(list(ASP))) -> list(ASP).
interleave(List) ->
    _pipe = List,
...
-spec shuffle_pair_unwrap_loop(list({float(), ATR}), list(ATR)) -> list(ATR).
...
-spec do_shuffle_by_pair_indexes(list({float(), ATV})) -> list({float(), ATV}).
...
-spec shuffle(list(ATO)) -> list(ATO).
shuffle(List) ->
    _pipe = List,
...
-spec max_loop(list(AUF), fun((AUF, AUF) -> gleam@order:order()), AUF) -> AUF.
...
-spec max(list(ATY), fun((ATY, ATY) -> gleam@order:order())) -> {ok, ATY} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-spec build_reservoir_loop(
    list(AUU),
    integer(),
    gleam@dict:dict(integer(), AUU)
) -> {gleam@dict:dict(integer(), AUU), list(AUU)}.
...
-spec build_reservoir(list(AUP), integer()) -> {gleam@dict:dict(integer(), AUP),
    list(AUP)}.
...
-spec sample(list(AUG), integer()) -> list(AUG).
sample(List, N) ->
...
-spec permutation_zip(list(APC), list(APC), list(list(APC))) -> list(list(APC)).
