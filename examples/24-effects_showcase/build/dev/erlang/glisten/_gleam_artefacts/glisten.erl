...
-type message(PEF) :: {packet, bitstring()} | {user, PEF}.
...
-type connection(PEG) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(PEG))}.

-opaque next(PEH, PEI) :: {continue,
        PEH,
        gleam@option:option(gleam@erlang@process:selector(PEI))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(PEJ, PEK) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(PEK)) -> {PEJ,
            gleam@option:option(gleam@erlang@process:selector(PEK))}),
        fun((PEJ, message(PEK), connection(PEK)) -> next(PEJ, message(PEK))),
        gleam@option:option(fun((PEJ) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(PFK) -> next(PFK, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(PFO, PFP), gleam@erlang@process:selector(PFP)) -> next(PFO, PFP).
...
?DOC(false).
-spec convert_next(next(PGD, PGE)) -> glisten@internal@handler:next(PGD, PGE).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(PGJ, PGK), fun((PGK) -> PGN)) -> next(PGJ, PGN).
...
-spec map_user_selector(gleam@erlang@process:selector(message(PGQ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(PGQ)).
...
-spec convert_loop(
    fun((PGV, message(PGW), connection(PGW)) -> next(PGV, message(PGW)))
) -> fun((PGV, glisten@internal@handler:loop_message(PGW), glisten@internal@handler:connection(PGW)) -> glisten@internal@handler:next(PGV, glisten@internal@handler:loop_message(PGW))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(PHB)) -> {PHD,
        gleam@option:option(gleam@erlang@process:selector(PHB))})
) -> fun((glisten@internal@handler:connection(PHB)) -> {PHD,
    gleam@option:option(gleam@erlang@process:selector(PHB))}).
...
-spec new(
    fun((connection(PHJ)) -> {PHL,
        gleam@option:option(gleam@erlang@process:selector(PHJ))}),
    fun((PHL, message(PHJ), connection(PHJ)) -> next(PHL, message(PHJ)))
) -> builder(PHL, PHJ).
new(On_init, Loop) ->
...
-spec with_close(builder(PHS, PHT), fun((PHS) -> nil)) -> builder(PHS, PHT).
...
-spec with_pool_size(builder(PHY, PHZ), integer()) -> builder(PHY, PHZ).
...
?DOC(false).
-spec with_http2(builder(PIE, PIF)) -> builder(PIE, PIF).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(PIQ, PIR)) -> builder(PIQ, PIR).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(PIW, PIX), binary(), binary()) -> builder(PIW, PIX).
...
-spec bind(builder(PIK, PIL), binary()) -> builder(PIK, PIL).
