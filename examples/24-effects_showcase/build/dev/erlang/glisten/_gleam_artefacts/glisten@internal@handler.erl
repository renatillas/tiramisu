...
-type message(OJR) :: {internal, internal_message()} | {user, OJR}.

-type loop_message(OJS) :: {packet, bitstring()} | {custom, OJS}.

-type loop_state(OJT, OJU) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(OJU)),
        glisten@transport:transport(),
        OJT}.

-type connection(OJV) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(OJV))}.

-type next(OJW, OJX) :: {continue,
        OJW,
        gleam@option:option(gleam@erlang@process:selector(OJX))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(OJY, OJZ) :: {handler,
        glisten@socket:socket(),
        fun((OJY, loop_message(OJZ), connection(OJZ)) -> next(OJY, loop_message(OJZ))),
        fun((connection(OJZ)) -> {OJY,
            gleam@option:option(gleam@erlang@process:selector(OJZ))}),
        gleam@option:option(fun((OJY) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(OKM) -> next(OKM, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(OKQ, OKR), gleam@erlang@process:selector(OKR)) -> next(OKQ, OKR).
...
?DOC(false).
-spec start(handler(any(), OLG)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(OLG)))} |
