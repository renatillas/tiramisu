...
-spec push_key({ok, AGOR} | {error, get_error()}, binary()) -> {ok, AGOR} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {AGQW, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AGQW, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {AGRH, list(binary())}} | {error, parse_error()},
    fun((AGRH, list(binary())) -> {ok, AGRK} | {error, parse_error()})
) -> {ok, AGRK} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {AGRP, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AGRP, list(binary())}} | {error, parse_error()}.
