...
-spec be_ok({ok, MDN} | {error, any()}) -> MDN.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, MDS}) -> MDS.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(MDV)) -> MDV.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
