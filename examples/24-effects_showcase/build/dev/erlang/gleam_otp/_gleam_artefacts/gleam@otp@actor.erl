...
-type message(KBX) :: {message, KBX} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(KBY, KBZ) :: {continue,
        KBY,
        gleam@option:option(gleam@erlang@process:selector(KBZ))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(KCA, KCB) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        KCA,
        gleam@erlang@process:selector(message(KCB)),
        gleam@otp@system:debug_state(),
        fun((KCA, KCB) -> next(KCA, KCB))}.

-type started(KCC) :: {started, gleam@erlang@process:pid_(), KCC}.

-opaque initialised(KCD, KCE, KCF) :: {initialised,
        KCD,
        gleam@option:option(gleam@erlang@process:selector(KCE)),
        KCF}.

-opaque builder(KCG, KCH, KCI) :: {builder,
        fun((gleam@erlang@process:subject(KCH)) -> {ok,
                initialised(KCG, KCH, KCI)} |
            {error, binary()}),
        integer(),
        fun((KCG, KCH) -> next(KCG, KCH)),
        gleam@option:option(gleam@erlang@process:name(KCH))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(KCJ) :: {ack, {ok, KCJ} | {error, binary()}} |
...
-spec continue(KCO) -> next(KCO, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(KDA, KDB), gleam@erlang@process:selector(KDB)) -> next(KDA, KDB).
...
-spec initialised(KDH) -> initialised(KDH, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(KDM, any(), KDO),
    gleam@erlang@process:selector(KDS)
) -> initialised(KDM, KDS, KDO).
...
-spec returning(initialised(KDX, KDY, any()), KED) -> initialised(KDX, KDY, KED).
...
-spec new(KEH) -> builder(KEH, KEI, gleam@erlang@process:subject(KEI)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(KEN)) -> {ok, initialised(KEP, KEN, KEQ)} |
        {error, binary()})
) -> builder(KEP, KEN, KEQ).
...
-spec on_message(builder(KEZ, KFA, KFB), fun((KEZ, KFA) -> next(KEZ, KFA))) -> builder(KEZ, KFA, KFB).
...
-spec named(builder(KFK, KFL, KFM), gleam@erlang@process:name(KFL)) -> builder(KFK, KFL, KFM).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(KFZ))) -> gleam@erlang@process:selector(message(KFZ)).
...
-spec receive_message(self(any(), KFV)) -> message(KFV).
...
-spec initialise_actor(
    builder(any(), any(), KGS),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, KGS} | {error, binary()})
...
-spec start(builder(any(), any(), KHF)) -> {ok, started(KHF)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(KHM), KHM) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(KHO),
    integer(),
    fun((gleam@erlang@process:subject(KHQ)) -> KHO)
) -> KHQ.
