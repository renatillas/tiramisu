...
-type child_specification(KVF) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(KVF)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(KVG)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(KVG).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(KVL)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(KVL).
supervisor(Start) ->
...
-spec significant(child_specification(KVQ), boolean()) -> child_specification(KVQ).
...
-spec timeout(child_specification(KVT), integer()) -> child_specification(KVT).
timeout(Child, Ms) ->
...
-spec restart(child_specification(KVW), restart()) -> child_specification(KVW).
...
-spec map_data(child_specification(KVZ), fun((KVZ) -> KWB)) -> child_specification(KWB).
