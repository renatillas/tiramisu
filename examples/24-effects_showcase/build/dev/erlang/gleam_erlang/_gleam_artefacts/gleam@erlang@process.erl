...
-opaque subject(IDQ) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(IDQ)}.

-type name(IDR) :: any() | {gleam_phantom, IDR}.

-type do_not_leak() :: any().

-type selector(IDS) :: any() | {gleam_phantom, IDS}.
...
-spec 'receive'(subject(IEQ), integer()) -> {ok, IEQ} | {error, nil}.
...
-spec receive_forever(subject(IEY)) -> IEY.
...
-spec selector_receive(selector(IFC), integer()) -> {ok, IFC} | {error, nil}.
...
-spec selector_receive_forever(selector(IFG)) -> IFG.
...
-spec map_selector(selector(IFI), fun((IFI) -> IFK)) -> selector(IFK).
map_selector(A, B) ->
...
-spec merge_selector(selector(IFM), selector(IFM)) -> selector(IFM).
merge_selector(A, B) ->
...
-spec select_map(selector(IFX), subject(IFZ), fun((IFZ) -> IFX)) -> selector(IFX).
...
-spec select(selector(IFT), subject(IFT)) -> selector(IFT).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> IGH)
) -> selector(IGH).
...
-spec select_other(selector(IGL), fun((gleam@dynamic:dynamic_()) -> IGL)) -> selector(IGL).
...
-spec deselect(selector(IGC), subject(any())) -> selector(IGC).
...
-spec select_specific_monitor(selector(IGX), monitor(), fun((down()) -> IGX)) -> selector(IGX).
...
-spec select_monitors(selector(IHA), fun((down()) -> IHA)) -> selector(IHA).
...
-spec select_trapped_exits(selector(IFQ), fun((exit_message()) -> IFQ)) -> selector(IFQ).
...
-spec deselect_specific_monitor(selector(IHD), monitor()) -> selector(IHD).
...
-spec send_after(subject(IHZ), integer(), IHZ) -> timer().
...
-spec perform_call(
    subject(IHG),
    fun((subject(IHI)) -> IHG),
    fun((selector(IHI)) -> {ok, IHI} | {error, nil})
) -> IHI.
...
-spec call(subject(IHN), integer(), fun((subject(IHP)) -> IHN)) -> IHP.
...
-spec call_forever(subject(IHR), fun((subject(IHT)) -> IHR)) -> IHT.
