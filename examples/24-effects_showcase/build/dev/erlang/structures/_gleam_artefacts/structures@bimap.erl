...
-opaque bi_map(AMGK, AMGL) :: {bi_map,
        gleam@dict:dict(AMGK, AMGL),
        gleam@dict:dict(AMGL, AMGK)}.
...
-spec delete(bi_map(AMHE, AMHF), AMHE) -> bi_map(AMHE, AMHF).
delete(From, Key) ->
    Value = get(From, Key),
...
-spec delete_val(bi_map(AMHJ, AMHK), AMHK) -> bi_map(AMHJ, AMHK).
...
-spec insert(bi_map(AMGN, AMGO), AMGN, AMGO) -> bi_map(AMGN, AMGO).
