...
-type client_message(UID) :: {mount,
        ...
        lustre@vdom@vnode:element(UID)} |
    {reconcile, integer(), lustre@vdom@patch:patch(UID)} |
...
?DOC(false).
-spec reconcile(lustre@vdom@patch:patch(UIM)) -> client_message(UIM).
reconcile(Patch) ->
    {reconcile, 1, Patch}.
