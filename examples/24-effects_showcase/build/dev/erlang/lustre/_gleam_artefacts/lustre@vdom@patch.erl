...
-type diff(UDB) :: {diff, patch(UDB), lustre@vdom@events:events(UDB)}.

-type patch(UDC) :: {patch,
        integer(),
        integer(),
        list(change(UDC)),
        list(patch(UDC))}.

-type change(UDD) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(UDD)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(UDD)), integer()}.

-file("src/lustre/vdom/patch.gleam", 73).
?DOC(false).
-spec new(integer(), integer(), list(change(UDE)), list(patch(UDE))) -> patch(UDE).
...
?DOC(false).
-spec add_child(patch(UEH), patch(UEH)) -> patch(UEH).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(UDO)),
    list(lustre@vdom@vattr:attribute(UDO))
) -> change(UDO).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(UDY)) -> change(UDY).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 123).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(UEB)), integer()) -> change(UEB).
