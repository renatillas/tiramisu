...
-opaque effect(RRD) :: {effect,
        list(fun((actions(RRD)) -> nil)),
        list(fun((actions(RRD)) -> nil)),
        list(fun((actions(RRD)) -> nil))}.

-type actions(RRE) :: {actions,
        fun((RRE) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(RRE)) -> nil),
...
-spec do_comap_select(
    actions(RSO),
    gleam@erlang@process:selector(RSQ),
    fun((RSQ) -> RSO)
) -> nil.
...
-spec do_comap_actions(actions(RSK), fun((RSM) -> RSK)) -> actions(RSM).
...
-spec do_map(list(fun((actions(RSE)) -> nil)), fun((RSE) -> RSH)) -> list(fun((actions(RSH)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(RSA), fun((RSA) -> RSC)) -> effect(RSC).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((RSS) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(RSS)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((RRH) -> nil)) -> nil)) -> effect(RRH).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((RRJ) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RRJ).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((RRL) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RRL).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((RRP) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(RRP))
) -> effect(RRP).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(RRW))) -> effect(RRW).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
