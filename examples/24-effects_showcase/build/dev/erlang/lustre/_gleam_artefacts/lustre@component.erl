...
-opaque config(WDB) :: {config,
        boolean(),
        boolean(),
        boolean(),
        list({binary(), fun((binary()) -> {ok, WDB} | {error, nil})}),
        list({binary(), gleam@dynamic@decode:decoder(WDB)}),
        list({binary(), gleam@dynamic@decode:decoder(WDB)}),
        boolean(),
        gleam@option:option(fun((binary()) -> WDB)),
        gleam@option:option(WDB),
        gleam@option:option(fun((binary()) -> WDB))}.

-opaque option(WDC) :: {option, fun((config(WDC)) -> config(WDC))}.

-file("src/lustre/component.gleam", 133).
?DOC(false).
-spec new(list(option(WDD))) -> config(WDD).
new(Options) ->
...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, WDH} | {error, nil})) -> option(WDH).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(WDL)) -> option(WDL).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(WDO)) -> option(WDO).
...
-spec on_form_autofill(fun((binary()) -> WDT)) -> option(WDT).
...
-spec on_form_reset(WDV) -> option(WDV).
on_form_reset(Msg) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_form_restore(fun((binary()) -> WDX)) -> option(WDX).
...
?DOC(false).
-spec to_server_component_config(config(WEF)) -> lustre@runtime@server@runtime:config(WEF).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(WEI)),
    list(lustre@vdom@vnode:element(WEI))
) -> lustre@vdom@vnode:element(WEI).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(WEO)),
    list(lustre@vdom@vnode:element(WEO))
) -> lustre@vdom@vnode:element(WEO).
