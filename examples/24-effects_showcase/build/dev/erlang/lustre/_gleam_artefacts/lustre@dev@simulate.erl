...
-opaque app(XHP, XHQ, XHR) :: {app,
        fun((XHP) -> {XHQ, lustre@effect:effect(XHR)}),
        fun((XHQ, XHR) -> {XHQ, lustre@effect:effect(XHR)}),
        fun((XHQ) -> lustre@vdom@vnode:element(XHR))}.

-opaque simulation(XHS, XHT) :: {simulation,
        fun((XHS, XHT) -> {XHS, lustre@effect:effect(XHT)}),
        fun((XHS) -> lustre@vdom@vnode:element(XHT)),
        list(event(XHT)),
        XHS,
        lustre@vdom@vnode:element(XHT)}.

-type event(XHU) :: {dispatch, XHU} |
...
-spec simple(
    fun((XHV) -> XHW),
    fun((XHW, XHX) -> XHW),
    fun((XHW) -> lustre@vdom@vnode:element(XHX))
) -> app(XHV, XHW, XHX).
...
-spec application(
    fun((XIC) -> {XID, lustre@effect:effect(XIE)}),
    fun((XID, XIE) -> {XID, lustre@effect:effect(XIE)}),
    fun((XID) -> lustre@vdom@vnode:element(XIE))
) -> app(XIC, XID, XIE).
...
-spec start(app(XIL, XIM, XIN), XIL) -> simulation(XIM, XIN).
start(App, Args) ->
...
-spec message(simulation(XIT, XIU), XIU) -> simulation(XIT, XIU).
...
-spec problem(simulation(XJZ, XKA), binary(), binary()) -> simulation(XJZ, XKA).
...
-spec model(simulation(XKF, any())) -> XKF.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), XKK)) -> lustre@vdom@vnode:element(XKK).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), XKP)) -> list(event(XKP)).
history(Simulation) ->
...
-spec click(simulation(XJG, XJH), lustre@dev@query:'query'()) -> simulation(XJG, XJH).
...
-spec input(simulation(XJM, XJN), lustre@dev@query:'query'(), binary()) -> simulation(XJM, XJN).
