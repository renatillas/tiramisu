...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(XSN)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XSN)),
    list(lustre@vdom@vnode:element(XSN))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XSN)),
    list(lustre@vdom@vnode:element(XSN))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(XSF)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XSF)),
    list(lustre@vdom@vnode:element(XSF))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(XQL)),
    list({binary(), lustre@vdom@vnode:element(XQL)})
) -> lustre@vdom@vnode:element(XQL).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(XQR)),
    list({binary(), lustre@vdom@vnode:element(XQR)})
) -> lustre@vdom@vnode:element(XQR).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(XQX)})) -> lustre@vdom@vnode:element(XQX).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(XRB)),
    list({binary(), lustre@vdom@vnode:element(XRB)})
) -> lustre@vdom@vnode:element(XRB).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 148).
-spec ol(
    list(lustre@vdom@vattr:attribute(XRH)),
    list({binary(), lustre@vdom@vnode:element(XRH)})
) -> lustre@vdom@vnode:element(XRH).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 155).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(XRN)),
    list({binary(), lustre@vdom@vnode:element(XRN)})
) -> lustre@vdom@vnode:element(XRN).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 162).
-spec tbody(
    list(lustre@vdom@vattr:attribute(XRT)),
    list({binary(), lustre@vdom@vnode:element(XRT)})
) -> lustre@vdom@vnode:element(XRT).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 169).
-spec dl(
    list(lustre@vdom@vattr:attribute(XRZ)),
    list({binary(), lustre@vdom@vnode:element(XRZ)})
) -> lustre@vdom@vnode:element(XRZ).
