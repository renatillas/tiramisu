...
-opaque app(WNC, WND, WNE) :: {app,
        fun((WNC) -> {WND, lustre@effect:effect(WNE)}),
        fun((WND, WNE) -> {WND, lustre@effect:effect(WNE)}),
        fun((WND) -> lustre@vdom@vnode:element(WNE)),
        lustre@component:config(WNE)}.
...
-type runtime(WNF) :: any() | {gleam_phantom, WNF}.
...
-spec component(
    fun((WOE) -> {WOF, lustre@effect:effect(WOG)}),
    fun((WOF, WOG) -> {WOF, lustre@effect:effect(WOG)}),
    fun((WOF) -> lustre@vdom@vnode:element(WOG)),
    list(lustre@component:option(WOG))
) -> app(WOE, WOF, WOG).
...
-spec application(
    fun((WNV) -> {WNW, lustre@effect:effect(WNX)}),
    fun((WNW, WNX) -> {WNW, lustre@effect:effect(WNX)}),
    fun((WNW) -> lustre@vdom@vnode:element(WNX))
) -> app(WNV, WNW, WNX).
...
-spec element(lustre@vdom@vnode:element(WNI)) -> app(any(), nil, WNI).
element(View) ->
    application(
...
-spec simple(
    fun((WNO) -> WNP),
    fun((WNP, WNQ) -> WNP),
    fun((WNP) -> lustre@vdom@vnode:element(WNQ))
) -> app(WNO, WNP, WNQ).
...
-spec do_start(app(WOY, any(), WPA), binary(), WOY) -> {ok, runtime(WPA)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec send(runtime(WPX), lustre@runtime@server@runtime:message(WPX)) -> nil.
...
-spec dispatch(WQA) -> lustre@runtime@server@runtime:message(WQA).
dispatch(Msg) ->
...
-spec start(app(WOP, any(), WOR), binary(), WOP) -> {ok, runtime(WOR)} |
    {error, error()}.
...
-spec start_server_component(app(WPH, any(), WPJ), WPH) -> {ok, runtime(WPJ)} |
    {error, error()}.
