...
-type attribute(QTH) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(QTH)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        boolean(),
...
-type handler(QTI) :: {handler, boolean(), boolean(), QTI}.
...
?DOC(false).
-spec merge(list(attribute(QTX)), list(attribute(QTX))) -> list(attribute(QTX)).
...
?DOC(false).
-spec compare(attribute(QUE), attribute(QUE)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 94).
?DOC(false).
-spec prepare(list(attribute(QTS))) -> list(attribute(QTS)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(QTN)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    boolean(),
