...
-spec find_matching_in_list(
    list(lustre@vdom@vnode:element(WUC)),
    selector(),
...
-spec find_direct_child(
    lustre@vdom@vnode:element(WTX),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTX), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_all_matching_in_list(
    list(lustre@vdom@vnode:element(WVK)),
    selector()
) -> list(lustre@vdom@vnode:element(WVK)).
...
-spec find_all_direct_children(lustre@vdom@vnode:element(WVG), selector()) -> list(lustre@vdom@vnode:element(WVG)).
...
-spec find_all_in_list(list(lustre@vdom@vnode:element(WVB)), 'query'()) -> list(lustre@vdom@vnode:element(WVB)).
...
-spec find_all(lustre@vdom@vnode:element(WUT), 'query'()) -> list(lustre@vdom@vnode:element(WUT)).
find_all(Root, Query) ->
    case Query of
        {find_element, Selector} ->
...
-spec find_all_in_children(lustre@vdom@vnode:element(WUX), 'query'()) -> list(lustre@vdom@vnode:element(WUX)).
...
-spec find_all_descendants_in_list(
    list(lustre@vdom@vnode:element(WVT)),
    selector()
) -> list(lustre@vdom@vnode:element(WVT)).
...
-spec find_all_descendants(lustre@vdom@vnode:element(WVP), selector()) -> list(lustre@vdom@vnode:element(WVP)).
...
-spec find_in_list(
    list(lustre@vdom@vnode:element(WTR)),
    'query'(),
    lustre@vdom@path:path(),
    integer()
...
?DOC(false).
-spec find_path(
    lustre@vdom@vnode:element(WTH),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTH), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_in_children(
    lustre@vdom@vnode:element(WTM),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WTM), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find(lustre@vdom@vnode:element(WTC), 'query'()) -> {ok,
        lustre@vdom@vnode:element(WTC)} |
    {error, nil}.
find(Root, Query) ->
...
-spec find_descendant_in_list(
    list(lustre@vdom@vnode:element(WUN)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(WUN), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_descendant(
    lustre@vdom@vnode:element(WUI),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(WUI), lustre@vdom@path:path()}} |
    {error, nil}.
