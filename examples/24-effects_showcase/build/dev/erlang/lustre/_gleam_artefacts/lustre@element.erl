...
-spec fragment(list(lustre@vdom@vnode:element(SSO))) -> lustre@vdom@vnode:element(SSO).
fragment(Children) ->
...
-spec unsafe_raw_html(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(SSS)),
    binary()
) -> lustre@vdom@vnode:element(SSS).
...
-spec map(lustre@vdom@vnode:element(SSW), fun((SSW) -> SSY)) -> lustre@vdom@vnode:element(SSY).
map(Element, F) ->
