...
-type state(UXG, UXH) :: {state,
        gleam@erlang@process:subject(message(UXH)),
        gleam@erlang@process:selector(message(UXH)),
        gleam@erlang@process:selector(message(UXH)),
        UXG,
        fun((UXG, UXH) -> {UXG, lustre@effect:effect(UXH)}),
        fun((UXG) -> lustre@vdom@vnode:element(UXH)),
        config(UXH),
        lustre@vdom@vnode:element(UXH),
        lustre@vdom@events:events(UXH),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(UXH)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(UXH)) -> nil))}.

-type config(UXI) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, UXI} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(UXI)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(UXI))}.

-type message(UXJ) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(UXJ))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(UXJ))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(UXJ)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(UXJ)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(UXJ))} |
    {effect_dispatched_message, UXJ} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, UYN} | {error, nil})),
    binary(),
    binary()
) -> {ok, UYN} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(UYU)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(UZA)),
    lustre@effect:effect(UZA)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(UZE)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(UZE)) -> nil)),
    lustre@runtime@transport:client_message(UZE)
) -> nil.
...
?DOC(false).
-spec loop(state(UXX, UXY), message(UXY)) -> gleam@otp@actor:next(state(UXX, UXY), message(UXY)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    {UXN, lustre@effect:effect(UXO)},
    fun((UXN, UXO) -> {UXN, lustre@effect:effect(UXO)}),
    fun((UXN) -> lustre@vdom@vnode:element(UXO)),
    config(UXO)
) -> {ok, gleam@erlang@process:subject(message(UXO))} |
