...
-type diff(UMT) :: {diff,
        lustre@vdom@patch:patch(UMT),
        lustre@vdom@events:events(UMT)}.

-type attribute_change(UMU) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(UMU)),
        list(lustre@vdom@vattr:attribute(UMU)),
        lustre@vdom@events:events(UMU)}.
...
-spec diff_attributes(
    ...
    lustre@vdom@events:events(UNW),
    list(lustre@vdom@vattr:attribute(UNW)),
    list(lustre@vdom@vattr:attribute(UNW)),
    list(lustre@vdom@vattr:attribute(UNW)),
    list(lustre@vdom@vattr:attribute(UNW))
) -> attribute_change(UNW).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(UNA)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UNA)),
    list(lustre@vdom@vnode:element(UNA)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UNA)),
    ...
    integer(),
    lustre@vdom@path:path(),
    list(lustre@vdom@patch:change(UNA)),
    list(lustre@vdom@patch:patch(UNA)),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@events:events(UNA)
) -> diff(UNA).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@events:events(UMV),
    lustre@vdom@vnode:element(UMV),
    lustre@vdom@vnode:element(UMV)
) -> diff(UMV).
diff(Events, Old, New) ->
    do_diff(
