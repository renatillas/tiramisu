...
-spec method_override(gleam@http@request:request(PUF)) -> gleam@http@request:request(PUF).
...
-spec sort_keys(list({binary(), PVW})) -> list({binary(), PVW}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, PUQ} | {error, any()}) -> {ok, PUQ} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((PVE) -> {ok, PVF} | {error, any()}),
    binary()
) -> fun((PVE) -> {ok, PVF} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((PUY, bitstring()) -> {ok, PUY} |
        {error, gleam@http@response:response(body())}),
    PUY
) -> {ok, {gleam@option:option(buffered_reader()), integer(), PUY}} |
    {error, gleam@http@response:response(body())}.
