...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGCK))
) -> glint:command(AGCK).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(AGDB)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AGDB} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, AGDB} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGDK))
) -> glint:command(AGDK).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGCE))
) -> glint:command(AGCE).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGCR))
) -> glint:command(AGCR).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AGCX))
) -> glint:command(AGCX).
