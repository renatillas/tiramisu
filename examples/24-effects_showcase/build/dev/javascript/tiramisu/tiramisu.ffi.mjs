...
/**
 * AudioManager - Manages all audio state (sources, groups, context, loading)
 * Encapsulates mutable audio state for the Web Audio API
 */
export class AudioManager {
  ...
  constructor() {
     ...
     * @type {Map<string, THREE.Audio|THREE.PositionalAudio>}
     */
    ...
    /**
     * Pending playbacks (queued until AudioContext resumes)
     * @type {Array<{id: any, source: THREE.Audio, fadeDuration: number}>}
     */
    this.pendingPlaybacks = [];

    /**
     * Three.js AudioLoader for loading audio files
     * @type {THREE.AudioLoader}
     */
    this.audioLoader = new THREE.AudioLoader();

    // Set up event listeners for user interaction (to resume AudioContext)
    this._initAudioContextListeners();
  }

  /**
   * Initialize audio context resume listeners
   * @private
   */
  _initAudioContextListeners() {
    ...
    if (typeof document !== 'undefined' && !window.__tiramisu.initialized.audioContextListeners) {
      ...
      ['click', 'touchstart', 'keydown'].forEach(eventType => {
        document.addEventListener(eventType, () => this.resumeAudioContext(), { once: true });
      });
    }
  }
  ...
  resumeAudioContext() {
    ...
    if (listener.context.state === 'suspended') {
      listener.context.resume().then(() => {
        ...
        this.pendingPlaybacks.forEach(({ id, source, fadeDuration }) => {
  ...
  unregisterAudioSource(id) {
    ...
    const source = this.audioSources.get(idStr);
    ...
    this.audioSources.delete(idStr);
  }

  /**
   * Get an audio source by ID
   * @param {any} id - Unique identifier
   * @returns {THREE.Audio | THREE.PositionalAudio | undefined}
   */
  getAudioSource(id) {
    return this.audioSources.get(idToString(id));
  }
  ...
  setGroupVolume(group, volume) {
    ...
    this.audioSources.forEach((source) => {
      if (source.userData && source.userData.group === group) {
        ...
        const effectiveVolume = this.mutedGroups.has(group) ? 0.0 : baseVolume * clampedVolume;
  ...
  unmuteGroup(group) {
    ...
    const groupVolume = this.groupVolumes.get(group) || 1.0;
    this.audioSources.forEach((source) => {
      if (source.userData && source.userData.group === group) {
        const baseVolume = source.userData.baseVolume || 1.0;
...
// FFI helper functions for Gleam to call AudioManager methods

/**
 * Create a new AudioManager instance (wrapper for Gleam FFI)
 ...
 * @param {AudioManager} manager - The AudioManager instance
 * @param {any} id - Unique identifier
 * @param {THREE.Audio | THREE.PositionalAudio} source - Audio source
 * @returns {void}
 */
export function audioManagerRegisterSource(manager, id, source) {
  manager.registerAudioSource(id, source);
}
 ...
 * @returns {void}
 */
export function audioManagerUnregisterSource(manager, id) {
  manager.unregisterAudioSource(id);
}
 ...
 * @returns {THREE.Audio | THREE.PositionalAudio | undefined}
 */
export function audioManagerGetSource(manager, id) {
  return manager.getAudioSource(id);
}

/**
 * Set group volume via the manager
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} group - Group name
 * @param {number} volume - Volume (0.0 to 1.0)
 * @returns {void}
 */
export function audioManagerSetGroupVolume(manager, group, volume) {
  manager.setGroupVolume(group, volume);
}

/**
 * Get group volume from the manager
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} group - Group name
 * @returns {number} Volume (0.0 to 1.0)
 */
export function audioManagerGetGroupVolume(manager, group) {
  return manager.getGroupVolume(group);
}

/**
 * Mute a group via the manager
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} group - Group name
 * @returns {void}
 */
export function audioManagerMuteGroup(manager, group) {
  manager.muteGroup(group);
}

/**
 * Unmute a group via the manager
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} group - Group name
 * @returns {void}
 */
export function audioManagerUnmuteGroup(manager, group) {
  manager.unmuteGroup(group);
}

/**
 * Load audio via the manager
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} url - URL of the audio file
 * @returns {Promise<Result<AudioBuffer, string>>}
 */
export function audioManagerLoadAudio(manager, url) {
  return manager.loadAudio(url);
}
 ...
 * @returns {void}
 */
export function audioManagerDestroy(manager) {
  manager.destroy();
}
...
import { getAudioListener as getAudioListenerFFI } from './threejs.ffi.mjs';
...
// ============================================================================
// AUDIO PLAYBACK - Audio source control (play, pause, stop, volume, etc.)
// ============================================================================
...
/**
 * Register an audio source (UPDATED to use manager)
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} id - Unique identifier
 * @param {THREE.Audio | THREE.PositionalAudio} source - Audio source
 */
export function registerAudioSource(manager, id, source) {
  manager.registerAudioSource(id, source);
}

/**
 * Unregister an audio source (UPDATED to use manager)
 * @param {AudioManager} manager - The AudioManager instance
 * @param {string} id - Unique identifier
 */
export function unregisterAudioSource(manager, id) {
  manager.unregisterAudioSource(id);
}

/**
 * Get an audio source by ID (UPDATED to use manager)
 * @param {AudioManager} manager - The AudioManager instance
 * @param {any} id - Audio source ID
 * @returns {THREE.Audio | THREE.PositionalAudio | undefined}
 */
function getAudioSource(manager, id) {
  return manager.getAudioSource(id);
}
 ...
 */
export function getAudioSourceForCleanup(manager, id) {
  return getAudioSource(manager, id);
}

/**
 * Create and configure an audio source from a scene Audio node (UPDATED to use manager)
 * @param {AudioManager} manager - The AudioManager instance
 ...
 */
export function playAudio(manager, id, buffer, config, audioType) {
  // Remove existing source if it exists
  const existingSource = manager.getAudioSource(id);
  if (existingSource) {
    stopAudio(manager, id);
    unregisterAudioSource(manager, id);
  }
  ...
  if (config.group && config.group[0]) {
    ...
    if (groupName) {
      ...
      const groupVolume = manager.getGroupVolume(groupName);
      const isMuted = manager.mutedGroups.has(groupName);
  ...
  // Register the source
  registerAudioSource(manager, id, source);
  ...
  if (initialStateName === 'Stopped' || initialStateName === 'Paused') {
    ...
    applyAudioState(manager, id, source, config);
  }
 ...
 * @param {AudioConfig} config - Audio configuration
 */
function applyAudioState(manager, id, source, config) {
  ...
  if (stateName === 'Playing' && previousState !== 'Playing') {
    ...
    if (listener.context.state === 'suspended') {
      ...
      manager.pendingPlaybacks.push({ id, source, fadeDuration });
    } else {
        ...
        if (fadeDuration > 0 && !isResumingFromPause) {
          ...
          if (isResumingFromPause) {
            const groupVolume = source.userData.group
              ? manager.getGroupVolume(source.userData.group)
              : 1.0;
            const isMuted = source.userData.group
              ? manager.mutedGroups.has(source.userData.group)
              : false;
            const baseVolume = source.userData.baseVolume || 1.0;
 ...
 */
export function stopAudio(manager, sourceId) {
  const source = getAudioSource(manager, sourceId);
  if (source) {
    if (source.isPlaying) {
      source.stop();
    }
    unregisterAudioSource(manager, sourceId);
  }
}
 ...
 */
export function updateAudioConfig(manager, id, config) {
  const source = getAudioSource(manager, id);
  if (!source) {
    return;
  }
  ...
  if (source.userData.group) {
    const groupVolume = manager.getGroupVolume(source.userData.group);
    const isMuted = manager.mutedGroups.has(source.userData.group);
    const effectiveVolume = isMuted ? 0.0 : config.volume * groupVolume;
    source.setVolume(effectiveVolume);
  } else {
    source.setVolume(config.volume);
 ...
 * @param {number} volume - Volume (0.0 to 1.0)
 */
export function setGroupVolume(manager, group, volume) {
  manager.setGroupVolume(group, volume);
}
 ...
 * @returns {number} Volume (0.0 to 1.0)
 */
export function getGroupVolume(manager, group) {
  return manager.getGroupVolume(group);
}
 ...
 * @param {string} group - Group name
 */
export function muteGroup(manager, group) {
  manager.muteGroup(group);
}
 ...
 * @param {string} group - Group name
 */
export function unmuteGroup(manager, group) {
  manager.unmuteGroup(group);
}
